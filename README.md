...
### 自动更新（推荐）
已启用 GitHub Actions 自动同步，每天凌晨 2 点自动检查 Hermes 上游更新并保护破甲配置。
...
# 只更新依赖，不同步上游
.\seagull-update.ps1 -SkipUpstream

# 强制重装所有依赖
.\seagull-update.ps1 -ForceUpdate
```

**更新后自动保护以下配置：**
- ✅ AGENTS.md（破甲指令集）
- ✅ skills/（所有技能模块）
- ✅ system-prompt.md（基础替换层）
...
### 方法三：验证脚本
```powershell
.\activate-seagull.ps1  # 检查环境并显示激活状态
```
